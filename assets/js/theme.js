import { t, getAvailableLangs, getLanguageLabel, getCurrentLang, switchLanguage, ensureLanguageBundle, getPublicLanguageOptions } from './i18n.js?v=press-system-v3.4.139';
import { getThemeRegion } from './theme-regions.js?v=press-system-v3.4.139';
import { createSiteFeatureContext, siteFeatureContextEnabled } from './site-features.js?v=press-system-v3.4.139';
...
function ensurePressComponents() {
  ...
  try {
    if (!registry[COMPONENTS_READY]) {
      registry[COMPONENTS_READY] = import('./components.js?v=press-system-v3.4.139').catch((err) => {
    ...
    return import('./components.js?v=press-system-v3.4.139').catch((err) => {
      console.warn('[theme] Failed to load press components', err);
      return null;
    });
  }
