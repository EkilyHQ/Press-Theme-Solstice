...
for (const filePath of ['theme/modules/interactions.js', 'theme/modules/layout.js']) {
  const solsticeSource = await readFile(path.join(SCRIPT_DIR, '..', filePath), 'utf8');
  ...
  assert.deepEqual(
    ...
    withoutLocations(
      scanJavaScriptSource({
        filePath,
        source: formattedSolsticeSource,
        wrapperNames: policy.wrappers.map(({ name }) => name)
      })
    ),
    `the exact Solstice sink inventory must remain stable when Prettier formats ${filePath}`
  );
}
