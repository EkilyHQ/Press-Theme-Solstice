...
for (const filePath of ['theme/modules/interactions.js', 'theme/modules/layout.js']) {
  const solsticeSource = await readFile(new URL(`../${filePath}`, import.meta.url), 'utf8');
  const formattedSolsticeSource = await prettier.format(solsticeSource, {
    parser: 'babel',
    printWidth: 120,
    semi: true,
    singleQuote: true,
  ...
  assert.deepEqual(
    createBaseline(scanJavaScriptSource({ filePath, source: solsticeSource })),
    createBaseline(scanJavaScriptSource({ filePath, source: formattedSolsticeSource })),
    `the exact Solstice debt baseline must remain stable when Prettier formats ${filePath}`
  );
}
