...
assert.equal(policy.scope, 'Solstice first-party theme source and repository tooling');
...
assert.equal(eslintDebtBaseline.diagnostics.length, 98);
...
assert.deepEqual(htmlPolicy.expectedKinds, {
  ...
  'html-wrapper-call:showToc': 2,
  'innerHTML-write': 23
});
assert.equal(htmlPolicy.approved.length, 28, 'all direct writes and reviewed wrapper calls must remain classified');
...
assert.deepEqual(
  ...
  ['renderLoader', 'renderStaticView', 'showToc']
);
